...
### ✅ 반드시 사용해야 하는 경우
1. **재고 관리**: 상품 재고 차감, 티켓팅 (선착순 이벤트)
2. **포인트/잔액 차감**: 사용자 포인트, 쿠폰 차감
3. **좌석 예약**: 영화관, 콘서트 좌석 예약 (중복 예약 방지)
4. **순번 발행**: 주문 번호, 티켓 번호 등 유니크 값 생성
5. **동시 수정 방지**: 게시글, 문서 동시 편집 시 충돌 방지

### 🔧 전략 선택 가이드
| 상황 | 권장 전략 | 이유 |
|------|----------|------|
| 충돌 빈도 높음 (재고 1개) | 비관적 락 | 충돌이 확실하므로 처음부터 락 획득 |
| 충돌 빈도 낮음 (재고 많음) | 낙관적 락 | 성능 우선, 충돌 시 재시도 |
| Scale-out 환경 (여러 서버) | 분산 락 (Redis) | 서버 간 동기화 필요 |
| 읽기 전용 (조회만) | 락 불필요 | 수정이 없으면 불필요 |

### ⚠️ 주의사항
- **데드락 위험**: 락 획득 순서를 통일하거나 타임아웃 설정 필수
- **성능 저하**: 락 범위를 최소화하고, 락 보유 시간을 짧게 유지
- **재시도 로직**: 낙관적 락 사용 시 재시도 전략 필수
