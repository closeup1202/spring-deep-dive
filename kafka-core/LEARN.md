...
Spring Kafka 기본값은 **BATCH**. 실무에서 주로 사용하는 모드 비교:

| AckMode | 커밋 시점 | Acknowledgment 파라미터 | 특성 |
|---------|-----------|------------------------|------|
| **BATCH** (기본값) | poll() 배치 전체 처리 후 자동 커밋 | 불필요 | 간단하지만 배치 중간 실패 시 성공한 레코드도 재처리 |
| **RECORD** | 각 레코드 리스너 완료 후 자동 커밋 | 불필요 | 단순 구현에 적합, 레코드 단위 커밋 |
| **MANUAL_IMMEDIATE** | `ack.acknowledge()` 호출 즉시 커밋 | 필수 | 처리 성공 여부를 직접 제어, at-least-once 보장 |
| **MANUAL** | `ack.acknowledge()` 후 다음 poll() 직전 커밋 | 필수 | MANUAL_IMMEDIATE보다 커밋 시점 느슨, I/O 약간 감소 |

```
실무 선택 기준:
  BATCH    → 간단한 서비스, 중복 재처리 허용되는 경우
  RECORD   → Acknowledgment 없이 레코드 단위 커밋 원할 때
  MANUAL_IMMEDIATE → 처리 성공 후에만 정확히 커밋해야 할 때 (현재 설정)
```

### KafkaProducerConfig 핵심 포인트

```java
// 클라이언트 식별 (브로커 로그 / Kafka UI에서 어느 인스턴스인지 구별)
...
// 버퍼 백프레셔 (기본값 60,000ms는 너무 길어 브로커 장애 시 스레드가 1분 블로킹)
config.put(ProducerConfig.MAX_BLOCK_MS_CONFIG, 5_000);  // 5초 → 빠른 실패(fail-fast)
```

### KafkaConsumerConfig 핵심 포인트

```java
// 클라이언트 식별 (K8s 환경: "order-service-consumer-" + podName)
...
// Fetch 최적화 (기본값 1byte는 즉시 응답 → 잦은 소규모 fetch)
config.put(ConsumerConfig.FETCH_MIN_BYTES_CONFIG, 1_024);  // 1KB 이상 모여야 응답
config.put(ConsumerConfig.FETCH_MAX_WAIT_MS_CONFIG, 500);  // 조건 미충족 시 최대 대기

// Static Group Membership (재시작 시 리밸런싱 방지)
// config.put(ConsumerConfig.GROUP_INSTANCE_ID_CONFIG, "order-consumer-0");

// 컨테이너 설정
factory.getContainerProperties().setAckMode(AckMode.MANUAL_IMMEDIATE);
factory.getContainerProperties().setPollTimeout(3_000);     // 종료 응답성 향상 (기본: 5,000ms)
factory.getContainerProperties().setShutdownTimeout(30_000); // Graceful Shutdown (기본: 10,000ms)
factory.setConcurrency(3); // 파티션 수 이하로
```

### fetch.min.bytes / fetch.max.wait.ms 동작

```
브로커 동작:
  Consumer → fetch 요청 → 브로커
  브로커: 데이터가 fetch.min.bytes(1KB) 이상 모였는가?
    Yes → 즉시 응답
    No  → fetch.max.wait.ms(500ms) 대기 후 응답 (데이터 없어도)

기본값(1byte)의 문제:
  저처리량 환경: 메시지 1건만 있어도 즉시 응답 → poll() 호출마다 소규모 fetch
  → 네트워크 왕복 증가, 브로커 CPU 부하 증가

1KB로 조정 시:
  최소 1KB 모일 때까지 기다렸다가 한 번에 전달 → 네트워크 효율 향상
  최대 대기 500ms → 트래픽 적어도 0.5초 내에는 반드시 응답받음
```

### max.block.ms — 프로듀서 백프레셔

```
기본값 60,000ms(60초) 문제:
  브로커 다운 → Record Accumulator 가득 참 → send() 스레드 1분 블로킹
  → API 서버라면 모든 요청 스레드가 60초씩 대기 → 서비스 전체 마비

5,000ms(5초) 설정:
  5초 내 버퍼 공간 확보 안 되면 → BufferExhaustedException 발생
  → 호출자가 즉시 예외 인지 → Outbox 패턴으로 재발행 가능
  → 서비스 자체는 계속 동작 가능
```
...
```
SIGTERM 수신 (K8s Pod 종료 등)
  → 컨테이너: 새 poll() 중단, 현재 처리 중인 레코드 완료 대기
  → setShutdownTimeout(30,000ms) 내로 완료되면 정상 종료
  → 30초 초과 시 강제 종료 (처리 중 레코드는 재처리됨)

setPollTimeout(3,000ms):
  poll() 대기 중 SIGTERM → 최대 3초 내 poll() 루프 이탈
  → Graceful Shutdown 시작
  기본값 5초면 최악의 경우 SIGTERM 후 5초를 poll()에서 소비
```
...
| `AckMode.RECORD` 또는 `MANUAL_IMMEDIATE` | 기본 BATCH보다 세밀한 커밋 제어 |
| `CooperativeStickyAssignor` 설정 | 리밸런싱 중 처리 중단 최소화 |
| `fetch.min.bytes` 조정 (1byte → 1KB) | 저처리량 환경에서 잦은 소규모 fetch 방지 |
| `max.poll.interval.ms` > 최대 처리 시간 | 무거운 처리 중 리밸런싱 방지 |
| `setPollTimeout` 단축 (5s → 3s) | SIGTERM 수신 후 종료 응답성 향상 |
| `setShutdownTimeout` 설정 (기본 10s 확인) | 롤링 배포 시 처리 중 레코드 안전하게 완료 |
| `group.instance.id` 설정 (Static Membership) | 재시작 시 불필요한 리밸런싱 방지 |
