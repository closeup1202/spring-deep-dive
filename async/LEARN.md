...
### ✅ 반드시 사용해야 하는 경우
1. **이메일/SMS 발송**: 사용자 응답을 기다릴 필요가 없는 작업
2. **로그 저장/분석**: 메인 로직과 무관한 로깅 작업
3. **외부 API 호출**: 느린 외부 API를 병렬로 호출하여 응답 시간 단축
4. **대용량 파일 처리**: 파일 업로드/변환 등 시간이 오래 걸리는 작업
5. **이벤트 발행**: 메인 트랜잭션과 분리된 후속 작업 처리

### ⚠️ 사용하면 안 되는 경우
- **트랜잭션 컨텍스트 유지 필요**: @Transactional과 @Async를 같은 메서드에 사용하면 트랜잭션이 적용되지 않음
- **결과를 즉시 받아야 하는 경우**: CompletableFuture로 결과를 받을 순 있지만, 동기 방식이 더 적합
- **순서 보장이 중요한 작업**: 비동기는 순서를 보장하지 않음
